...
category             -- 50 種の事項別分類コード（API 実形式は "1".."50"。1 憲法 … 50 外事）
...
TABLE law
  ...
  law_num           TEXT             NOT NULL  -- 例: 昭和二十二年政令第十六号（非ユニーク。下記注）
  law_num_era       FK -> era
...
> **注（全件取り込みで判明）**: `law_num` は一意ではない。異なる `law_id`（人事院規則・省令など、
> 発令主体が異なる）が同一の `law_num` 文字列を持つ実データがあるため、当初の `UNIQUE` 制約は
> 撤廃し非ユニーク索引にした（migration 0003）。一意キーは `law_id`（PK）のみ。
