...
- 全件取り込み: `file_section=1` の Zip をローカルに展開 → 法令ごとに XML → DB UPSERT。長尺のため **Procrastinate の親ジョブ → 法令単位の子ジョブ**に分割する（§11.7）。
- 差分取り込み: 日次で `file_section=3&update_date=YYYYMMDD` を取得（過去 3 か月のみ参照可）。Procrastinate の `@periodic(cron="0 3 * * *")` で起動。
