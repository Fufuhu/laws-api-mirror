...
**方針 A: 不透明 XML として保持（最小実装）**

- `law_node` には `kind = 'QuoteStruct'` の 1 ノードだけを作り、内部要素は分解しない。
- `raw_xml` に XML 断片を、`text_plain` にインライン要素を剥がしたプレーンテキストを保持。
- 検索: `text_plain` を pg_bigm / tsvector で索引化（§5）。ヒット位置は QuoteStruct ノード単位までで、内部の条番号までは特定しない。
- レンダリング: `/law_data` レスポンスでは `raw_xml` をそのまま埋め戻す。
- **長所**: 実装シンプル、XSD/未知要素変化に強い、ストレージ最小、パース処理が軽い。`xs:any` の本来の意図と整合。
- **短所**: 引用先法令へのリンクなし、検索ヒット位置の粒度が粗い、引用関係グラフを別途構築しない限り取れない。

**方針 B: 完全構造化**

- QuoteStruct の中身も再帰的に `law_node` ツリーに展開（Article / Paragraph / Sentence ... を子ノードとして格納）。
- `law_node.path` には `QuoteStruct` を中継ラベルとして含める（例: `MainProvision.Article_5.Paragraph_1.QuoteStruct.Article_3.Paragraph_1`）。
- **長所**: 細粒度な検索ヒット位置、引用された条文の構造的アクセス、引用関係の自動抽出が容易。
- **短所**:
  - `xs:any` のためスキーマ保証なし、未知要素のハンドリングが必要（無視 / 保留 / エラーの判定）。
  - 本文ツリーと引用ツリーが `law_node` 上で混在し、本文の `elm` パス解決クエリで除外条件 (`path ~ '*QuoteStruct*'` の否定)が必要になりパフォーマンス劣化。
  - ストレージ・パースコスト増（引用ブロックは法令によっては大量）。
  - レンダリング時に「引用ブロック」を識別して囲み直す再構築ロジックが必要。

**方針 C: ハイブリッド（A + 抽出影テーブル）**

- 本体は方針 A と同じく `QuoteStruct` 1 ノードに raw_xml で格納。
- 別途 **`quote_extract` テーブル**で、引用された法令の情報と引用テキストを抽出・正規化:

  ```
  TABLE quote_extract
    id                BIGSERIAL PRIMARY KEY
    law_node_id       BIGINT FK -> law_node(id) ON DELETE CASCADE  -- 親 QuoteStruct ノード
    quoted_law_num    TEXT                       -- 引用先の法令番号（抽出できれば）
    quoted_law_id     VARCHAR(15)                -- 解決できれば law_id
    quoted_path_text  TEXT                       -- "Article_3-Paragraph_1" 等（断片構造）
    text_plain        TEXT
...
- 本文ツリー（`law_node`）はクリーンに保ち、引用は別テーブルで扱う。
- 検索は本文と引用を OR 結合できる。
- **長所**: 本文と引用を分離しつつ、引用関係の分析（§11.5「引用関係の抽出」）と統合可能。引用先法令へのリンクが取れる。
- **短所**: 抽出ロジックの精度依存。引用 XML 断片はフォーマットが不定で、引用先 law_num の抽出に失敗するケースがある（その場合は `quoted_law_num = NULL` で記録）。

**方針 D: 段階導入（A → C）**

- 初期リリースは方針 A（最小実装）でリリース。
- §11.5「引用関係の抽出」と連動して方針 C の `quote_extract` を後段で追加。
- 既存 `law_node` のスキーマ変更なしで C を上乗せできるため、移行コストが小さい。

#### 11.9.2 評価軸

| 観点 | A: 不透明 | B: 完全構造化 | C: ハイブリッド | D: 段階導入 (A→C) |
|---|---|---|---|---|
| 実装コスト | ◎ 最小 | × 大（未知要素ハンドリング、再構築） | ○ 中（抽出器のみ） | ◎→○ |
| 検索ヒット位置の粒度 | △ QuoteStruct ノードまで | ◎ 引用内の条文単位 | ○ `quote_extract` 単位 | △→○ |
| 引用関係の分析 | × 別途実装 | ○ 自然に可能 | ◎ 専用テーブル | △→◎ |
| 本文ツリーの清潔性 | ◎ | △ 本文と混在 | ◎ | ◎ |
| ストレージコスト | ◎ | × | ○ | ◎→○ |
| XSD 変化への耐性 | ◎ | △ | ◎ | ◎ |
...
3. 方針 B は **本文ツリーの清潔性を損なう**点で本プロジェクトの設計（`law_node.path` を `elm` 解決の主軸とする）と相性が悪い。`xs:any` を `law_node` のシリアスな構造として扱うべきではないという判断。
4. 方針 C 単独より、段階導入のほうがスケジュールリスクが低い。

#### 11.9.4 残課題（方針 D 採用時）

- 方針 A のテキスト抽出ロジック（`raw_xml` → `text_plain`）の品質確認（外字・縦書き記号の扱い）。
- 方針 C 移行時の `quote_extract` 抽出器の設計:
  - 引用 XML 断片から `LawNum` / `LawTitle` / 条番号をどう抽出するか
  - 抽出に失敗したエッジケースの記録（`quoted_law_num = NULL` ＋ `text_plain` のみ）
  - §11.5「引用関係の抽出」との統合スコープ
- 方針 D における移行時の既存データ再処理（全 QuoteStruct を再パースして `quote_extract` を生成するバックフィルジョブ）。

### 11.10 法令本文の追加ファイル形式（html / rtf / docx）

**位置付け**: 1st リリースでは非対応（§10-3 で確定）。将来 `/law_file/{file_type}` で対応する場合の検討メモ。

#### 11.10.1 想定ユースケース

- 法律の条文を **Word (docx)** で配布したい（法務部門・行政機関向け）。
- **RTF** で他システム（古い行政システム）に取り込みたい。
- **HTML** をブラウザ表示・印刷用に提供したい（e-Gov 本家と同等の体験）。

#### 11.10.2 生成方式の候補

| 形式 | 生成方式の候補 | 備考 |
|---|---|---|
| HTML | XSLT（法令XML → HTML） / Jinja2 テンプレ + 構造ノードトラバーサル | e-Gov 公式 XSLT は非公開（ヘルプ Q7「スタイルシートは提供していません」）。自前テンプレ必須。CSS で縦書き・ルビ対応。 |
| RTF | `python-docx` + RTF 変換 / `pypandoc` (Pandoc バイナリ依存) | フォーマット仕様が古く、表・ルビ・縦書きの再現精度に難あり。 |
| DOCX | `python-docx` で直接生成 / `pypandoc` | 表・段落・スタイル制御は可能。縦書きは Word 側設定。 |

#### 11.10.3 生成タイミング

- **オンデマンド生成（リクエスト時）**: シンプル、ストレージ不要。ただし大法令で数秒〜十数秒かかる場合があり、レイテンシが問題。
- **事前生成（取り込み時にバッチ）**: 全形式 × 全法令履歴を S3 に格納。ストレージは増えるがレスポンスは即時。
- **ハイブリッド（人気法令のみ事前生成、それ以外はオンデマンド + キャッシュ）**: 中規模運用に適合。

#### 11.10.4 設計上の留意点

- **レンダリングロジックは `rendering` モジュールに集約**（§3 のプロジェクト構成にすでに項目あり）。1st リリースでは XML / JSON レンダラのみ実装し、HTML / RTF / DOCX レンダラは同モジュールに後付けする。
- **`law_node` ツリーをトラバースするレンダラ**を抽象化しておけば、出力形式の追加は新しいレンダラ実装の追加だけで済む。
- **縦書き・ルビ・外字**: HTML は CSS Writing Modes + `<ruby>` で対応可能。RTF / DOCX は表現力に限界があり、e-Gov とは異なる見た目になる旨を API レスポンス or ドキュメントに明記する。
- e-Gov のヘルプにも「ダウンロードしたファイルは官報とはレイアウトが異なります」と明記されている前例があり、本プロジェクトでも同様の免責で運用可能。

#### 11.10.5 残課題（将来検討時）

- 各形式の品質要件（e-Gov 同等 vs 簡易版）の確定
- 事前生成 / オンデマンドの判断
- HTML テンプレートの設計と XSLT 代替の検討
- 外字（私用領域）の扱い：画像差し替え or Unicode 拡張領域マッピング
- DOCX の縦書き・段組（1段組 / 2段組 / 4段組）対応の要否

### 11.11 JSON `light` 形式のエッジケース突合せ

**位置付け**: 1st リリースでは **インタフェース互換のみ維持**し、レスポンス詳細は最も容易な実装を採用する方針（§10-9 で確定）。本節は **e-Gov 実 API とのバイト単位差分**を縮めるための将来対応メモ。

#### 11.11.1 既知のエッジケース

API ドキュメント（OpenAPI YAML）に明示されていないが、e-Gov 実 API の挙動を観察しないと正解が分からない領域。

1. **`Ruby` 要素のテキスト埋め込み**
   - 親要素（ベーステキスト）と `<Rt>`（振り仮名）の合成方法。
   - 例: `<Ruby>漢字<Rt>かんじ</Rt></Ruby>` を `light` でどう文字列化するか。
     - 案 A: ベースのみ → `"漢字"`
     - 案 B: 括弧付与 → `"漢字(かんじ)"`
     - 案 C: 別フィールド化 → `{"text": "漢字", "ruby": "かんじ"}`
   - **1st リリース実装**: 案 A（ベーステキストのみ。`text_plain` をそのまま採用、最も容易）。

2. **`ArithFormula` のテキスト化**
   - 数式（画像 + 代替テキスト）の `light` での扱い。
   - 例: 内部に `Fig` がある場合、`src` をどう表現するか。
   - **1st リリース実装**: `text_plain` に展開した結果（多くの場合は代替テキストなしで空文字 or 構造化フィールドなし）。

3. **`QuoteStruct` の埋め込み**
   - `xs:any` のため任意 XML が入る。`light` での文字列化。
   - **1st リリース実装**: §11.9 の方針 A（不透明 raw_xml）から `text_plain` を生成して埋め込み。

4. **`light` でフィールド化される属性の出力位置**
   - 「該当タグと同階層に別要素として格納する場合あり」と OpenAPI に記載。
   - 具体的なキー名（`Num` をそのまま使うか、`@Num` のような prefix を付けるか）。
   - **1st リリース実装**: キー名は属性名そのまま（`Num`, `AmendLawNum`, `Extract`）。タグ名と衝突する場合は属性側を優先しない（タグキーを残す）。

5. **属性破棄の境界線**
   - `Paragraph.Num` だけ残して `Article.Num` / `Item.Num` を本当に落とすかどうか。
   - **1st リリース実装**: OpenAPI 明示の 3 属性（`AmendLawNum`, `Extract`, `Paragraph.Num`）のみフィールド化し、それ以外は破棄。

6. **空要素・null の表現**
   - `light` で子を持たない要素を `null` にするか、空配列にするか、キー自体を出さないか。
   - **1st リリース実装**: キー自体を出さない（minimal）。

7. **テキストノードと子要素の混在**
   - `mixed="true"` 要素（Sentence など）で、テキストと子要素が交互に並ぶ場合の配列順序保持。
   - **1st リリース実装**: `text_plain`（インライン要素を剥がした結果）を単一文字列として埋め込む。順序情報は失われるが、ベーステキストの読みやすさを優先。

#### 11.11.2 将来の合わせ込みプロセス

将来 e-Gov 実 API との一致度を高める場合のステップ:

1. 代表法令 10〜20 件について、e-Gov 実 API の `light` レスポンスを録画してフィクスチャ化。
2. 本実装のレスポンスと **差分比較**（snapshot test）。
3. 差分の各項目について、(a) e-Gov に合わせる (b) 仕様の解釈差として独自仕様を維持 を判断。
4. (a) の項目だけ `rendering` 層に合わせ込みロジックを追加し、`light_compat_level` のような切替フラグで段階提供。

#### 11.11.3 注意

- 1st リリース時点での `light` レスポンスは **e-Gov とバイト単位で一致しない**ことを **API ドキュメント or README に明記**する。
- 互換テストの自動化（実 API レスポンスの定点観測）は、将来合わせ込みを実施する際の前提として早い段階で仕込んでおくと良い。

### 11.12 XSD バージョン管理運用

**位置付け**: §10-10 で「XSD は将来変わる前提で運用フローを仕組み化する」と確定。本節は具体的な運用設計メモ。

**前提**: 現行は法令標準 XML スキーマ v3.0（2020-11-24 公開、`XMLSchemaForJapaneseLaw_v3.xsd`）。`Law` ルート要素にバージョン番号属性は無く、XML 自体からは v3 か否か判別できない。

#### 11.12.1 想定される変更タイプ

| 変更タイプ | 例 | 影響範囲 |
|---|---|---|
| 新要素の追加 | `<Annex>` 等 | `node_kind` lookup、`rendering` 層の分岐 |
| 既存要素への新属性 | `Sentence.Lang` 等 | `law_node.attrs JSONB` で吸収 or 専用カラム化判断 |
| 属性値域の拡大 | `Era` に新元号 | 参照テーブル（`era`）への追記 |
| 要素名／構造の非互換変更 | `Subitem1` → `SubItem1`、`Paragraph` の入れ子ルール変更等 | 大規模対応：取り込み・`elm` パス解決・レンダリング全般 |
| 必須属性の追加・任意化 | `Article.Num` の必須化等 | 検証ポリシー、取り込み時のフォールバック |

#### 11.12.2 アップグレード検知

XSD の更新は **能動的に検知する仕組み**を入れる:

- **方法 A**: `XMLSchemaForJapaneseLaw_v3.xsd` の `HEAD` リクエストを Procrastinate の `@periodic`（例: 週次）で実行し、`Last-Modified` / `ETag` の変化を検出してアラート。
- **方法 B**: XSD 本体の SHA256 を定期取得し、保存値と差分があれば通知。
- **方法 C**: e-Gov 法令検索のお知らせページを定期スクレイピング（補助的）。
- 第1候補は **B（SHA256 比較）**。`Last-Modified` は CDN 都合で揺れる可能性がある。
- 検知時の通知先: 開発チーム（Slack / メール等）。

#### 11.12.3 妥当性検証ポリシー（取り込み時）

`lxml.etree.XMLSchema` で `XMLSchemaForJapaneseLaw_v3.xsd` による検証を実施する際の挙動:

- **モード 1: 厳格**: 検証失敗で取り込み即停止、要修正までジョブ DLQ 行き。
- **モード 2: 警告**: 検証失敗をログ＋メトリクスに記録するが、取り込みは続行。`law_node` への投入は属性 `attrs` に追い込んで延命。
- **モード 3: 警告 + 閾値**: モード 2 と同じだが、検証失敗が **連続 X 件 or 同一バッチ内で Y%** を超えたら自動停止。

採用方針:

- **1st リリース: モード 3（警告 + 閾値、デフォルト Y=5%）**。e-Gov 側のスキーマ更新を早期検知しつつ、軽微なゆらぎでは止まらないバランス。
- 閾値は環境変数で調整可能にし、検知ノイズを見ながらチューニング。

#### 11.12.4 `node_kind` 追加の Alembic リビジョン運用

- **新要素検知の起点**:
  - 検知方法 A: 取り込み時に未知の要素を見つけたら `unknown_node_kinds` テーブル（観測用）に記録。
  - 検知方法 B: §11.13.2 のアップグレード検知後、新 XSD を `xmllint` 等で差分解析して未知要素を抽出。
- **マスタ追加リビジョンの手順**:
  1. 新 XSD を取得し、要素一覧を抽出するスクリプトを実行。
  2. 既存 `node_kind` テーブルとの差分を取り、追加要素のリストアップ。
  3. Alembic リビジョンとして `op.bulk_insert(node_kind_table, [...])` を発行。
  4. 必要に応じて `law_node` の専用カラム追加（`attrs JSONB` で吸収できるなら不要）。
  5. CI で `node_kind` 件数の sanity check を実施。
- マスタ追加は **新バージョン対応コードのリリースとセット**にする（順序: マイグレーション先行 → コードデプロイ）。

#### 11.12.5 過渡期の両バージョン対応

e-Gov が v3 → v4 移行する場合、**同 DB に両バージョンの法令データが混在する**可能性がある。

- **`law_xml` テーブルに `xsd_version` カラムを追加**して、取り込み時に検証成功した XSD バージョンを記録。
  ```
  ALTER TABLE law_xml ADD COLUMN xsd_version TEXT NOT NULL DEFAULT 'v3.0';
  ```
- 取り込みパイプラインで **複数 XSD を fallback 順に検証**する仕組み（v3 で失敗したら v4 で試す）。
- レスポンス層では `xsd_version` を内部メタとして保持するのみで、API レスポンスには出さない（e-Gov 互換維持）。

#### 11.12.6 コード・テストフィクスチャ追従

- `law_node.kind` を Python の `Literal` / `StrEnum` で **強型付けしない**（lookup テーブルとの結合で十分とする）。型定義の更新を毎回追わなくて済む。
- `rendering` 層で要素ごとの分岐がある場合、**未知要素は raw_xml をそのまま埋め戻す**フォールバックを必ず実装。
- 統合テスト用フィクスチャ（代表法令 10〜20 件）は **XSD バージョンタグ付きで管理**し、新バージョン対応時に追加・差し替えのワークフローを文書化。

#### 11.12.7 残課題

- アップグレード検知ジョブの実装（Procrastinate `@periodic`）。
- `unknown_node_kinds` 観測テーブルの DDL と書き込みロジック。
- 新 XSD 入手時の差分抽出スクリプト（`xmllint` ベース or 自作）。
- 過渡期対応の優先度判断：v4 が実際に発表されるまで「準備のみ・実装は後回し」とするか。

#### アウトプット

- XSD アップグレード検知ジョブ（Procrastinate `@periodic`）の実装メモ
- マスタ追加リビジョンのテンプレート（Alembic）
- バージョン追従 Runbook

### 11.13 参考: 不採用方式の予備メモ（ジョブキュー方式 A: SQS + 自前ワーカー）
