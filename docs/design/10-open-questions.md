...
3. ~~**html/rtf/docx ファイル形式の生成**~~ → **1st リリースでは非対応**: 1st リリースは検索 API の提供を主目的とするため、`/law_file/{file_type}` でサポートするのは **`xml` と `json` のみ**。`html` / `rtf` / `docx` をリクエストされた場合は **400 Bad Request** で `unsupported file_type` 相当のエラーを返す（API §7 に反映）。将来要件として追加する可能性はあり、検討メモを §11.11 に残置。
