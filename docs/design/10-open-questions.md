...
1. ~~**添付ファイルの保存先**~~ → **確定**: AWS S3 または S3 互換オブジェクトストレージに保存（DB の BYTEA には入れない）。MinIO はパブリックアーカイブ状態のため不採用。S3 互換ソフトウェアの具体選定と CDN 配信の要否は §11.2 で別途検討（現時点で CDN／署名付き URL は対象外）。
