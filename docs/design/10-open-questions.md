...
10. ~~**XSD バージョン管理**~~ → **方針確定（運用設計は §11.13）**: 「XSD は不変ではなく、いずれバージョンアップする」前提で、(a) アップグレード検知、(b) `node_kind` 追加の Alembic リビジョン運用、(c) 妥当性検証ポリシー（厳格／警告／警告＋閾値）、(d) 過渡期の両バージョン対応、(e) コード・テストフィクスチャ追従、を**運用フローとして仕組み化する**。具体的な手順設計は §11.13。
