...
- **統合**: testcontainers で Postgres を立て（`pg_bigm` / `ltree` / `textsearch_ja` 同梱イメージ、§2.10）、Alembic で最新スキーマを適用 → 法令 1 件投入 → 全エンドポイントを叩いて JSON/XML 双方を比較。testcontainers の概念整理と本プロジェクトでの fixture 構成は [`docs/guides/testcontainers.md`](../guides/testcontainers.md) を参照。
